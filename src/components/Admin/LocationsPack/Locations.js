...
export default function Locations() {
    ...
    function getItems(){
        axios.get('/api/get-locations').then(res=>{
                setItemsList(res.data.locations);
            setLoading(false);
        }).catch(err=>{
    ...
    const deleteItem = (e,item) =>{
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                axios.delete(`/api/delete-location/${item.id}`).then(res=>{
       ...
       { itemsList.map((item)=>{
            return (
                ...
                <div key={item.id} className="mx-0 grid grid-cols-12 text-center break-all">
                    ...
                    <div className='pt-3 col-span-9 md:col-span-6 text-start'>{item.title}</div>
...
<InboxIcon className="block h-20 w-20" aria-hidden="true"/>
            <h3 className='text-2xl font-bold'>Theres no Locations</h3>
  ...
  return (
    ...
    <div className="max-w-[80rem] p-5 mx-auto">
        <div className='rounded-lg shadow-lg'>
            <div className="flex justify-between items-center bg-gray-100 p-3 rounded-t-lg">
                <h5 className='mb-3 mb-0'>Locations ( {itemsList.length} )</h5>
                <button type='button' className='bg-blue-600 text-white p-2 rounded' onClick={()=>{setModalShow(true)}}>Add Location</button>
            </div>
            <div className="p-5">
    ...
    <AddLocationModal show={modalShow} hide={()=>{setModalShow(false);getItems()}} />
    <EditLocationModal show={editModalShow} hide={()=>{setEditModalShow(false);getItems()}} toedit={toEdit} />
    </>
  )
